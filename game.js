...
const keys = new Set(), templates = new Map(), enemies = [], mixers = [], blocks = [], wallMeshes = [], groundMeshes = [], groundSamples = new Map();
...
function mapGroundAt(x,z,referenceY=mapFloorY) { const key=`${Math.round(x*4)},${Math.round(z*4)},${Math.round(referenceY*2)}`; if(groundSamples.has(key))return groundSamples.get(key); const origin=new THREE.Vector3(x,referenceY+.7,z); groundRaycaster.set(origin,new THREE.Vector3(0,-1,0)); groundRaycaster.far=2.8; const hit=groundRaycaster.intersectObjects(groundMeshes,false).find((item)=>item.face&&item.point.y<=referenceY+.5&&item.point.y>=referenceY-2&&item.face.normal.clone().transformDirection(item.object.matrixWorld).y>.45); const y=hit?hit.point.y+.03:referenceY; groundSamples.set(key,y); return y; }
...
function intersectsMapWall(from,to) { const travel=new THREE.Vector3(to.x-from.x,0,to.z-from.z), distance=travel.length(); if(!distance||!wallMeshes.length)return false; travel.multiplyScalar(1/distance); const side=new THREE.Vector3(-travel.z,0,travel.x).multiplyScalar(.24); for(const offset of [new THREE.Vector3(),side,side.clone().multiplyScalar(-1)]){const origin=from.clone().add(offset);origin.y=from.y+.82;wallRaycaster.set(origin,travel);wallRaycaster.far=distance+.42;const hit=wallRaycaster.intersectObjects(wallMeshes,false).find((item)=>item.face&&Math.abs(item.face.normal.clone().transformDirection(item.object.matrixWorld).y)<.65);if(hit)return true;}return false;}
function findClearSpawn(x,z) { const offsets=[[0,0],[2,0],[-2,0],[0,2],[0,-2],[4,0],[-4,0],[0,4],[0,-4],[4,4],[-4,4],[4,-4],[-4,-4]]; for(const [dx,dz] of offsets){const p=new THREE.Vector3(x+dx,mapGroundAt(x+dx,z+dz),z+dz); if(canMove(p,p)&&![[1,0],[-1,0],[0,1],[0,-1]].some(([ox,oz])=>intersectsMapWall(p,p.clone().add(new THREE.Vector3(ox*.55,0,oz*.55)))))return p;}return new THREE.Vector3(x,mapGroundAt(x,z),z);}
...
function canMove(from,p){const radius=.38,bodyBottom=p.y+.12,bodyTop=p.y+1.5;if(!arenaBounds||p.x-radius<arenaBounds.min.x||p.x+radius>arenaBounds.max.x||p.z-radius<arenaBounds.min.z||p.z+radius>arenaBounds.max.z)return false;if(blocks.some(b=>p.x+radius>b.min.x&&p.x-radius<b.max.x&&p.z+radius>b.min.z&&p.z-radius<b.max.z&&b.max.y>bodyBottom&&b.min.y<bodyTop))return false;return !intersectsMapWall(from,p);}
