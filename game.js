...
const loader = new GLTFLoader(), clock = new THREE.Clock(), raycaster = new THREE.Raycaster(), wallRaycaster = new THREE.Raycaster();
const dracoLoader = new DRACOLoader();
dracoLoader.setDecoderPath('https://www.gstatic.com/draco/versioned/decoders/1.5.6/');
loader.setDRACOLoader(dracoLoader);
const keys = new Set(), templates = new Map(), enemies = [], mixers = [], blocks = [], wallMeshes = [];
...
function playerUpdate(dt){if(!game.live)return;const d=new THREE.Vector3(),f=new THREE.Vector3(-Math.sin(game.yaw),0,-Math.cos(game.yaw)),r=new THREE.Vector3(Math.cos(game.yaw),0,-Math.sin(game.yaw)),forward=keys.has('KeyW')||keys.has('ArrowUp'),back=keys.has('KeyS')||keys.has('ArrowDown'),left=keys.has('KeyA')||keys.has('ArrowLeft'),right=keys.has('KeyD')||keys.has('ArrowRight'),speed=(keys.has('ShiftLeft')||keys.has('ShiftRight'))?7.2:4.3;if(forward)d.add(f);if(back)d.sub(f);if(left)d.sub(r);if(right)d.add(r);if(d.lengthSq()){d.normalize().multiplyScalar(speed*dt);const xStep=player.position.clone();xStep.x+=d.x;xStep.y=mapGroundAt(xStep.x,xStep.z);if(canMove(player.position,xStep))player.position.copy(xStep);const zStep=player.position.clone();zStep.z+=d.z;zStep.y=mapGroundAt(zStep.x,zStep.z);if(canMove(player.position,zStep))player.position.copy(zStep)}player.rotation.y=game.yaw;}
