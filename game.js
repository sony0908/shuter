...
function canMove(p){if(Math.abs(p.x)>32.5||p.z>25.5||p.z<-25.5)return false;const probe=new THREE.Box3().setFromCenterAndSize(p,new THREE.Vector3(.65,1.7,.65));return !blocks.some(b=>b.intersectsBox(probe));}
