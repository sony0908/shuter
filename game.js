...
const loader = new GLTFLoader(), clock = new THREE.Clock(), raycaster = new THREE.Raycaster(), wallRaycaster = new THREE.Raycaster(), groundRaycaster = new THREE.Raycaster();
...
const profiles = [{ name: 'MÍNIMA', ratio: .45, far: 30, shadows: false, fog: .052 }, { name: 'BAJA', ratio: .65, far: 42, shadows: false, fog: .034 }, { name: 'MEDIA', ratio: .9, far: 60, shadows: true, fog: .022 }, { name: 'ALTA', ratio: 1.15, far: 80, shadows: true, fog: .016 }];
...
function mapGroundAt(x,z,referenceY=mapFloorY,depth=2.8) { const key=`${Math.round(x*4)},${Math.round(z*4)},${Math.round(referenceY*2)},${Math.round(depth)}`; if(groundSamples.has(key))return groundSamples.get(key); const origin=new THREE.Vector3(x,referenceY+.7,z); groundRaycaster.set(origin,new THREE.Vector3(0,-1,0)); groundRaycaster.far=depth; const hit=groundRaycaster.intersectObjects(groundMeshes,false).find((item)=>item.face&&item.point.y<=referenceY+.5&&item.point.y>=referenceY-depth&&item.face.normal.clone().transformDirection(item.object.matrixWorld).y>.45); const y=hit?hit.point.y+.03:referenceY; if(groundSamples.size>2400)groundSamples.clear(); groundSamples.set(key,y); return y; }
function addMapCollider(object) { const b=new THREE.Box3().setFromObject(object), s=b.getSize(new THREE.Vector3()), name=object.name.toLowerCase(); const isVertical=s.y>.8&&s.x>.15&&s.z>.15; const isLocal=s.x<14&&s.z<14; const isLongWall=Math.min(s.x,s.z)<2.4; const isWallMaterial=/(wall|brick|plaster|concrete|door|gate|fence|wood|beam|pylon|bus|barrel|crate|shop|stair|step)/.test(name); const isGroundMaterial=/(floor|ground|blacktop|sand|tile|concrete|wood|step|stair|street|dirt|cobble|pave)/.test(name); if(isVertical&&(isLocal||isLongWall))blocks.push(b); if(isWallMaterial)wallMeshes.push(object); if(isGroundMaterial)groundMeshes.push(object); }
function addPropCollider(prop) { if(prop)blocks.push(new THREE.Box3().setFromObject(prop)); }
function intersectsMapWall(from,to) { const travel=new THREE.Vector3(to.x-from.x,0,to.z-from.z), distance=travel.length(); if(!distance||!wallMeshes.length)return false; const key=`${Math.round(from.x*3)},${Math.round(from.z*3)},${Math.round(to.x*3)},${Math.round(to.z*3)},${Math.round(from.y*2)}`; if(wallSamples.has(key))return wallSamples.get(key); travel.multiplyScalar(1/distance); const side=new THREE.Vector3(-travel.z,0,travel.x).multiplyScalar(.24); let blocked=false; for(const offset of [new THREE.Vector3(),side,side.clone().multiplyScalar(-1)]){const origin=from.clone().add(offset);origin.y=from.y+.82;wallRaycaster.set(origin,travel);wallRaycaster.far=distance+.42;const hit=wallRaycaster.intersectObjects(wallMeshes,false).find((item)=>item.face&&Math.abs(item.face.normal.clone().transformDirection(item.object.matrixWorld).y)<.65);if(hit){blocked=true;break;}}if(wallSamples.size>1800)wallSamples.clear();wallSamples.set(key,blocked);return blocked;}
function findClearSpawn(x,z,scanFromTop=false) { const offsets=[[0,0],[2,0],[-2,0],[0,2],[0,-2],[4,0],[-4,0],[0,4],[0,-4],[4,4],[-4,4],[4,-4],[-4,-4]], reference=scanFromTop?arenaBounds.max.y:mapFloorY, depth=scanFromTop?arenaBounds.max.y-arenaBounds.min.y+4:2.8; for(const [dx,dz] of offsets){const p=new THREE.Vector3(x+dx,mapGroundAt(x+dx,z+dz,reference,depth),z+dz); if(canMove(p,p,false)&&![[1,0],[-1,0],[0,1],[0,-1]].some(([ox,oz])=>intersectsMapWall(p,p.clone().add(new THREE.Vector3(ox*.55,0,oz*.55)))))return p;}return new THREE.Vector3(x,mapGroundAt(x,z,reference,depth),z);}
async function assets() {
  // Load character/weapon templates (keep) + mirage map (replaces old industrial yard)
  await Promise.all([
    ['rifle','player-rifle.glb'],['zombie','zombie.glb'],
  ...
  arenaBounds = new THREE.Box3().setFromObject(mirage.scene);
  // Mirage's gameplay streets share this source elevation. Keeping the navigation
...
let weapon;
function enemy(x,z,n) { const t=templates.get('zombie'), model=cloneSkinned(t.root), spawn=findClearSpawn(x,z,true); fit(model,1.78); model.position.copy(spawn).add(new THREE.Vector3(0,.05,0)); const e={model,health:100 + game.round * 9,cooldown:0,speed:1.05+n*.035+game.round*.025,live:true,deadFor:null,mixer:null,clips:t.clips,action:null,navigationTimer:0}; model.traverse(c=>{if(c.isMesh)c.userData.enemy=e}); scene.add(model); if(t.clips.length){const m=new THREE.AnimationMixer(model);e.mixer=m;mixers.push(m);enemyAction(e,'Walk')} enemies.push(e); }
...
function canMove(from,p,precise=true){const radius=.38,bodyBottom=p.y+.12,bodyTop=p.y+1.5;if(!arenaBounds||p.x-radius<arenaBounds.min.x||p.x+radius>arenaBounds.max.x||p.z-radius<arenaBounds.min.z||p.z+radius>arenaBounds.max.z)return false;if(blocks.some(b=>p.x+radius>b.min.x&&p.x-radius<b.max.x&&p.z+radius>b.min.z&&p.z-radius<b.max.z&&b.max.y>bodyBottom&&b.min.y<bodyTop))return false;return !precise||!intersectsMapWall(from,p);}
function playerUpdate(dt){if(!game.live)return;const d=new THREE.Vector3(),f=new THREE.Vector3(-Math.sin(game.yaw),0,-Math.cos(game.yaw)),r=new THREE.Vector3(Math.cos(game.yaw),0,-Math.sin(game.yaw)),forward=keys.has('KeyW')||keys.has('ArrowUp'),back=keys.has('KeyS')||keys.has('ArrowDown'),left=keys.has('KeyA')||keys.has('ArrowLeft'),right=keys.has('KeyD')||keys.has('ArrowRight'),speed=(keys.has('ShiftLeft')||keys.has('ShiftRight'))?7.2:4.3;if(forward)d.add(f);if(back)d.sub(f);if(left)d.sub(r);if(right)d.add(r);if(d.lengthSq()){d.normalize().multiplyScalar(speed*dt);const xStep=player.position.clone();xStep.x+=d.x;xStep.y=mapGroundAt(xStep.x,xStep.z,player.position.y);if(canMove(player.position,xStep))player.position.copy(xStep);const zStep=player.position.clone();zStep.z+=d.z;zStep.y=mapGroundAt(zStep.x,zStep.z,player.position.y);if(canMove(player.position,zStep))player.position.copy(zStep)}player.rotation.y=game.yaw;}
