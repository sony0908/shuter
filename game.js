...
const game = { live: false, health: 150, maxHealth: 150, ammo: 30, reserve: 120, reloading: false, lastShot: 0, noticeTime: 0, quality: 0, yaw: 0, pitch: -.13 };
...
function world() {
  ...
  // A closed perimeter keeps the playable yard contained and hides the map boundary.
  cube([47, 8, .6], [0, 4, -18], { map: rustyMetal }); cube([47, 8, .6], [0, 4, 16], { map: rustyMetal });
  cube([.6, 8, 35], [-23, 4, -1], { map: rustyMetal }); cube([.6, 8, 35], [23, 4, -1], { map: rustyMetal });
...
async function assets() {
  ...
  const rifle=instance('rifle',[0,0,0],.6); rifle.position.set(.39,-.36,-.72); rifle.rotation.set(-.18,Math.PI,.02); camera.add(rifle);
